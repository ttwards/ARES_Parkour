...
if TYPE_CHECKING:
    ...
    from .actions_cfg import DelayedJointPositionActionCfg, DelayedJointVelocityActionCfg
...
class DelayedJointVelocityAction(JointVelocityAction):
    """Joint action term that applies the processed actions to the articulation's joints as velocity commands."""

    cfg: DelayedJointVelocityActionCfg
    """The configuration of the action term."""

    def __init__(self, cfg: DelayedJointVelocityActionCfg, env: ManagerBasedEnv):
        # initialize the action term
        super().__init__(cfg, env)
        # use default joint velocities as offset (typically zeros)
        if cfg.use_default_offset:
