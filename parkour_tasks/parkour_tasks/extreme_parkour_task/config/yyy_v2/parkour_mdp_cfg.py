...
from parkour_isaaclab.envs.mdp.parkour_actions import DelayedJointPositionActionCfg, DelayedJointVelocityActionCfg
...
class CommandsCfg:
    ...
    base_velocity = parkour_commands.ParkourCommandCfg(
        ...
        terrain_ranges_map={
            ...
            "parkour_hurdle": {
                "lin_vel_x": (0.4, 1.2),  # 栏架地形中等速度
                "heading": (-0.8, 0.8)
            },
            "parkour_step": {
                "lin_vel_x": (0.5, 1.4),  # 台阶地形较慢速度
                "heading": (-1.6, 1.6)
            },
            "parkour_wall": {
                "lin_vel_x": (0.8, 1.2),  # 墙壁地形中等速度
                "heading": (-1.6, 1.6)
            }
        },
...
class TeacherRewardsCfg:
    ...
    'LF_Wheel_link', 'RF_Wheel_link', 'LR_Wheel_link', 'RR_Wheel_link']
    ...
    reward_collision = RewTerm(
        ...
        weight=-4.5,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["base_link", ".*_HipA_link", ".*_HipF_link", ".*_Knee_link"]),
        },
    ...
    reward_cumulative_speed_error = RewTerm(
        ...
        params={
            ...
            "command_name": "base_velocity",
            # 新逻辑参数
            "speed_threshold": 0.4,      # 速度差阈值 (m/s)
            "time_threshold_s": 1.5,     # 连续超阈需维持时间 (s)
            "fast_clear_decay": 0.0,     # 低于阈值后立刻清零
            "normal_decay": 0.98,        # 激活状态下的缓慢遗忘（原来的 decay）
    ...
    #             ["LF_Wheel_link", "RR_Wheel_link"],  # 对角线配对（trot步态）
    #             ["RF_Wheel_link", "LR_Wheel_link"],  # 对角线配对（trot步态）
    #         ],
    #     },
    #     terrain_weight_map={
    #         "parkour_flat": 1.15,
...
class ActionsCfg:
    ...
    wheel_vel = DelayedJointVelocityActionCfg(
        asset_name="robot",
